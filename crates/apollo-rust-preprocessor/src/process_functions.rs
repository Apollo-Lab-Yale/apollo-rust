...
use bevy_egui::egui::{Color32, ComboBox, RichText, SidePanel, TopBottomPanel};
use bevy_egui::EguiContexts;
use apollo_rust_bevy::{ApolloBevyTrait, get_default_mesh_specs};
use apollo_rust_bevy::apollo_bevy_utils::chain::{BevyChainProximityVisualizerRaw, BevyChainSlidersEguiRaw, ChainMeshesRepresentation, ChainState, BevyChainLinkVisibilitySelectorRaw};
...
pub fn run_link_shapes_skips_module_process(chain_name: &str) {
    let ss = if let Some(s) = ResourcesRootDirectory::new_from_default_apollo_robots_dir()
        .get_subdirectory_option(chain_name)
    {
        s
    } else if let Some(s) = ResourcesRootDirectory::new_from_default_apollo_environments_dir()
        .get_subdirectory_option(chain_name)
    {
        s
    } else {
    ...
    match &link_shapes_skips_module_result {
        Ok(link_shapes_skips_module) => {
            full_convex_hulls_skips =
                dmatrix_from_2dvec(&link_shapes_skips_module.full_convex_hulls_skips.clone());
            ...
            full_bounding_spheres_skips =
                dmatrix_from_2dvec(&link_shapes_skips_module.full_bounding_spheres_skips.clone());
            decomposition_convex_hulls_skips = dmatrix_from_2dvec(
                &link_shapes_skips_module
                    .decomposition_convex_hulls_skips
                    .clone(),
            );
            decomposition_obbs_skips =
                dmatrix_from_2dvec(&link_shapes_skips_module.decomposition_obbs_skips.clone());
            decomposition_bounding_spheres_skips = dmatrix_from_2dvec(
                &link_shapes_skips_module
        ...
        Err(_) => {
            full_convex_hulls_skips = link_shapes_simple_skips_module
                .full_convex_hulls_simple_skips
                .clone();
            full_obbs_skips = link_shapes_simple_skips_module
                .full_obbs_simple_skips
                .clone();
            full_bounding_spheres_skips = link_shapes_simple_skips_module
                .full_bounding_spheres_simple_skips
                .clone();
            decomposition_convex_hulls_skips = link_shapes_simple_skips_module
                .decomposition_convex_hulls_simple_skips
                .clone();
            decomposition_obbs_skips = link_shapes_simple_skips_module
                .decomposition_obbs_simple_skips
                .clone();
            decomposition_bounding_spheres_skips = link_shapes_simple_skips_module
                .decomposition_bounding_spheres_simple_skips
                .clone();
        }
    }

    let mut original_full_convex_hulls_skips = link_shapes_simple_skips_module
        .full_convex_hulls_simple_skips
        .clone();
    let mut original_full_obbs_skips = link_shapes_simple_skips_module
        .full_obbs_simple_skips
        .clone();
    let mut original_full_bounding_spheres_skips = link_shapes_simple_skips_module
        .full_bounding_spheres_simple_skips
        .clone();
    let mut original_decomposition_convex_hulls_skips = link_shapes_simple_skips_module
        .decomposition_convex_hulls_simple_skips
        .clone();
    let mut original_decomposition_obbs_skips = link_shapes_simple_skips_module
        .decomposition_obbs_simple_skips
        .clone();
    let mut original_decomposition_bounding_spheres_skips = link_shapes_simple_skips_module
        .decomposition_bounding_spheres_simple_skips
        .clone();
    ...
    match &combined_robot {
        Ok(combined_robot) => {
            ...
            for (chain_idx, attached_robot) in combined_robot.attached_robots().iter().enumerate() {
                let ss =
                    ResourcesRootDirectory::new(s.root_directory.clone(), s.resources_type.clone())
                        .get_subdirectory(attached_robot.robot_name());

                let ss_convex_hull_meshes_module =
                    ApolloConvexHullMeshesModule::load_or_build(&ss, false).expect("error");
                let ss_convex_decomposition_meshes_module =
                    ApolloConvexDecompositionMeshesModule::load_or_build(&ss, false)
                        .expect("error");
                let ss_link_shapes_module = ApolloLinkShapesModule::from_mesh_modules(
                    &ss,
                    &ss_convex_hull_meshes_module,
                    &ss_convex_decomposition_meshes_module,
                );
                let ss_link_shapes_skips_module =
                    ApolloLinkShapesSkipsModule::load_or_build(&ss, false).expect("error");

                let num_full = ss_link_shapes_skips_module.full_convex_hulls_skips.len();
                let num_decomposition = ss_link_shapes_skips_module
                    .decomposition_convex_hulls_skips
                    .len();

                for i in 0..num_full {
                    let sub_link_idx_i = ss_link_shapes_module.full_shape_idx_to_link_idx[i];
                    let combined_link_idx_i = info
                        .sub_chain_and_link_idx_to_combined_chain_link_idx[chain_idx]
                        [sub_link_idx_i]
                        .clone();
                    let combined_shape_idx_i =
                        link_shapes_module.link_idx_to_full_shape_idx[combined_link_idx_i].unwrap();
                    for j in 0..num_full {
                        let sub_link_idx_j = ss_link_shapes_module.full_shape_idx_to_link_idx[j];
                        let combined_link_idx_j = info
                            .sub_chain_and_link_idx_to_combined_chain_link_idx[chain_idx]
                            [sub_link_idx_j]
                            .clone();
                        let combined_shape_idx_j = link_shapes_module.link_idx_to_full_shape_idx
                            [combined_link_idx_j]
                            .unwrap();

                        let sub_skip =
                            ss_link_shapes_skips_module.full_bounding_spheres_skips[i][j];
                        if sub_skip {
                            full_bounding_spheres_skips
                                [(combined_shape_idx_i, combined_shape_idx_j)] = true;
                            full_bounding_spheres_skips
                                [(combined_shape_idx_j, combined_shape_idx_i)] = true;
                            original_full_bounding_spheres_skips
                                [(combined_shape_idx_i, combined_shape_idx_j)] = true;
                            original_full_bounding_spheres_skips
                                [(combined_shape_idx_j, combined_shape_idx_i)] = true;
                        }
                        ...
                        if sub_skip {
                            full_convex_hulls_skips[(combined_shape_idx_i, combined_shape_idx_j)] =
                                true;
                            full_convex_hulls_skips[(combined_shape_idx_j, combined_shape_idx_i)] =
                                true;
                            original_full_convex_hulls_skips
                                [(combined_shape_idx_i, combined_shape_idx_j)] = true;
                            original_full_convex_hulls_skips
                                [(combined_shape_idx_j, combined_shape_idx_i)] = true;
                        }
                ...
                for i in 0..num_decomposition {
                    let (sub_link_idx_i, aa) = ss_link_shapes_module
                        .decomposition_shape_idx_to_link_idx_and_link_sub_idx[i];
                    let combined_link_idx_i = info
                        .sub_chain_and_link_idx_to_combined_chain_link_idx[chain_idx]
                        [sub_link_idx_i]
                        .clone();
                    let combined_shape_idxs_i = link_shapes_module
                        .link_idx_to_decomposition_shape_idxs[combined_link_idx_i]
                        .clone();
                    let combined_shape_idx_i = combined_shape_idxs_i[aa];
                    for j in 0..num_decomposition {
                        let (sub_link_idx_j, bb) = ss_link_shapes_module
                            .decomposition_shape_idx_to_link_idx_and_link_sub_idx[j];
                        let combined_link_idx_j = info
                            .sub_chain_and_link_idx_to_combined_chain_link_idx[chain_idx]
                            [sub_link_idx_j]
                            .clone();
                        let combined_shape_idxs_j = link_shapes_module
                            .link_idx_to_decomposition_shape_idxs[combined_link_idx_j]
                            .clone();
                        let combined_shape_idx_j = combined_shape_idxs_j[bb];

                        let sub_skip =
                            ss_link_shapes_skips_module.decomposition_bounding_spheres_skips[i][j];
                        if sub_skip {
                            decomposition_bounding_spheres_skips
                                [(combined_shape_idx_i, combined_shape_idx_j)] = true;
                            decomposition_bounding_spheres_skips
                                [(combined_shape_idx_j, combined_shape_idx_i)] = true;
                            original_decomposition_bounding_spheres_skips
                                [(combined_shape_idx_i, combined_shape_idx_j)] = true;
                            original_decomposition_bounding_spheres_skips
                                [(combined_shape_idx_j, combined_shape_idx_i)] = true;
                        }
                        ...
                        if sub_skip {
                            ...
                            decomposition_obbs_skips
                                [(combined_shape_idx_j, combined_shape_idx_i)] = true;
                            original_decomposition_obbs_skips
                                [(combined_shape_idx_i, combined_shape_idx_j)] = true;
                            original_decomposition_obbs_skips
                                [(combined_shape_idx_j, combined_shape_idx_i)] = true;
                        }
                        ...
                        let sub_skip =
                            ss_link_shapes_skips_module.decomposition_convex_hulls_skips[i][j];
                        if sub_skip {
                            decomposition_convex_hulls_skips
                                [(combined_shape_idx_i, combined_shape_idx_j)] = true;
                            decomposition_convex_hulls_skips
                                [(combined_shape_idx_j, combined_shape_idx_i)] = true;
                            original_decomposition_convex_hulls_skips
                                [(combined_shape_idx_i, combined_shape_idx_j)] = true;
                            original_decomposition_convex_hulls_skips
                                [(combined_shape_idx_j, combined_shape_idx_i)] = true;
                        }
    ...
    let link_shapes_distance_statistics_module_clone =
        link_shapes_distance_statistics_module.clone();
...
pub fn run_first_look_vis_module_process(chain_name: &str) {
    let ss = if let Some(s) = ResourcesRootDirectory::new_from_default_apollo_robots_dir()
        .get_subdirectory_option(chain_name)
    {
        s
        ...
        (move |mut exit: EventWriter<AppExit>,
            ...
            TopBottomPanel::bottom("bottom_panel")
                ...
                .show(egui_contexts.ctx_mut(), |ui| {
                    ui.horizontal(|ui| {
                        if ui
                            .button(RichText::new("Verified").color(Color32::GREEN).size(25.0))
                            .clicked()
                        {
                        ...
                        ui.separator();
                        if ui
                            .button(RichText::new("Not Verified").color(Color32::RED).size(25.0))
                            .clicked()
                        {
