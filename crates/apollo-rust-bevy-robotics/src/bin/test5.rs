...
fn main() {
    ...
    // let res =  c.fk(&V::new(&[0.0; 6]));
    // res[9].0.rotation.to_lie_group_h1().displacement().ln().vee()

    let i = InterpolatingSpline::new(
        vec![V::new(&[0.0; 6]), V::new(&[1.0; 6])],
        InterpolatingSplineType::Linear).to_timed_interpolator(5.0);
    ...
    let mut app = App::new()
        ...
        // .apollo_bevy_draw_frame(&ISE3q::new(I3::from_slices_euler_angles(&[0.2, 0.0, 0.5], &[0.,0.,0.])), 0.2);
