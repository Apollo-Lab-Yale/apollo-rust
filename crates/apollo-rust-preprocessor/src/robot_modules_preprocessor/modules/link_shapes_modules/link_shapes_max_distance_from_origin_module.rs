...
impl<P: ApolloPathBufTrait + Clone> PreprocessorModule<P>
    for ApolloLinkShapesMaxDistanceFromOriginModule
{
    ...
    fn build_raw(
        ...
        link_shapes_module
            ...
            .for_each(|x| match x {
                ...
                Some(full_shape_idx) => {
                    full_convex_hulls_maximum_distances.push(Some(
                        link_shapes_module.full_convex_hulls[*full_shape_idx]
                            .calculate_max_dis_from_origin_to_point_on_shape(),
                    ));
                    full_obbs_maximum_distances.push(Some(
                        link_shapes_module.full_obbs[*full_shape_idx]
                            .calculate_max_dis_from_origin_to_point_on_shape(),
                    ));
                    full_bounding_spheres_maximum_distances.push(Some(
                        link_shapes_module.full_bounding_spheres[*full_shape_idx]
                            .calculate_max_dis_from_origin_to_point_on_shape(),
                    ));
                }
            });
        ...
        link_shapes_module
            ...
            .for_each(|x| {
                ...
                x.iter().for_each(|decomposition_shape_idx| {
                    tmp1.push(
                        link_shapes_module.decomposition_convex_hulls[*decomposition_shape_idx]
                            .calculate_max_dis_from_origin_to_point_on_shape(),
                    );
                    tmp2.push(
                        link_shapes_module.decomposition_obbs[*decomposition_shape_idx]
                            .calculate_max_dis_from_origin_to_point_on_shape(),
                    );
                    tmp3.push(
                        link_shapes_module.decomposition_bounding_spheres[*decomposition_shape_idx]
                            .calculate_max_dis_from_origin_to_point_on_shape(),
                    );
                });
