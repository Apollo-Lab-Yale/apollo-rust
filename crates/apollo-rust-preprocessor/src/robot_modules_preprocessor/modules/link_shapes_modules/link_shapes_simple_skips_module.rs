...
impl<P: ApolloPathBufTrait + Clone> PreprocessorModule<P> for ApolloLinkShapesSimpleSkipsModule {
    ...
    fn build_raw(
        ...
        let link_shapes_distance_stats_module =
        ...
        let full_convex_hulls_simple_skips = get_simple_skips_from_link_shapes_distance_stats(
            &link_shapes_distance_stats_module.full_convex_hulls,
            &link_shapes_module,
            LinkShapeMode::Full,
        );
        let full_obbs_simple_skips = get_simple_skips_from_link_shapes_distance_stats(
            &link_shapes_distance_stats_module.full_obbs,
            &link_shapes_module,
            LinkShapeMode::Full,
        );
        let full_bounding_spheres_simple_skips = get_simple_skips_from_link_shapes_distance_stats(
            &link_shapes_distance_stats_module.full_bounding_spheres,
            &link_shapes_module,
            LinkShapeMode::Full,
        );
        let decomposition_convex_hulls_simple_skips =
            get_simple_skips_from_link_shapes_distance_stats(
                &link_shapes_distance_stats_module.decomposition_convex_hulls,
                &link_shapes_module,
                LinkShapeMode::Decomposition,
            );
        let decomposition_obbs_simple_skips = get_simple_skips_from_link_shapes_distance_stats(
            &link_shapes_distance_stats_module.decomposition_obbs,
            &link_shapes_module,
            LinkShapeMode::Decomposition,
        );
        let decomposition_bounding_spheres_simple_skips =
            get_simple_skips_from_link_shapes_distance_stats(
                &link_shapes_distance_stats_module.decomposition_bounding_spheres,
                &link_shapes_module,
                LinkShapeMode::Decomposition,
            );
...
pub(crate) fn get_simple_skips_from_link_shapes_distance_stats(
    stats: &LinkShapesDistanceStatistics,
    link_shapes_module: &ApolloLinkShapesModule,
    shape_mode: LinkShapeMode,
) -> Vec<Vec<bool>> {
    ...
    for i in 0..n {
        for j in 0..n {
            ...
            let a =
                link_shapes_module.get_link_idx_and_subcomponent_idx_from_shape_idx(i, &shape_mode);
            let b =
                link_shapes_module.get_link_idx_and_subcomponent_idx_from_shape_idx(j, &shape_mode);
            if a.0 == b.0 {
                skip = true;
            }
