...
use apollo_rust_proximity_parry::{ProximityLossFunction, ToProximityValue};
...
fn main() {
    ...
    let mut bvh = c.get_bvh::<BvhShapeAABB>(
        &V::new(&[0.0; 6]),
    ...
    println!(
        "{:?}",
        res.to_proximity_value(&ProximityLossFunction::Hinge { threshold: 1.0 }, 10.0)
    );

    let res = c.self_contact(
        &fk_res,
        LinkShapeMode::Decomposition,
        LinkShapeRep::ConvexHull,
        false,
