...
use apollo_rust_modules::robot_modules::urdf_module::{ApolloURDFJointType, ApolloURDFModule};
...
impl DOFModuleBuilders for ApolloDOFModule {
    fn build_from_urdf_module(
        ...
        urdf_module
            ...
            .iter()
            .enumerate()
            .for_each(|(i, j)| match &j.mimic {
                None => match j.joint_type {
...
impl<P: ApolloPathBufTrait + Clone> PreprocessorModule<P> for ApolloDOFModule {
    ...
    fn build_raw(
        ...
        let urdf_module = ApolloURDFModule::load_or_build(s, false).expect(
            "could not build ApolloDOFModule because ApolloURDFModule could not be built or loaded",
        );
