...
use crate::{create_generic_build_from_adjusted_robot, create_generic_build_from_combined_robot, create_generic_build_raw, RobotPreprocessorModule, RobotPreprocessorSingleRobotDirectory};
use crate::utils::progress_bar::ProgressBarWrapper;
use crate::CombinedRobot;
use crate::RobotPreprocessorRobotsDirectory;
...
pub trait OriginalMeshesModuleBuilders: Sized {
    fn build_from_urdf_module(s: &RobotPreprocessorSingleRobotDirectory, progress_bar: &mut ProgressBarWrapper) -> Result<Self, String>;
    ...
    fn build_from_urdf_module(s: &RobotPreprocessorSingleRobotDirectory, progress_bar: &mut ProgressBarWrapper) -> Result<Self, String> {
        ...
        return if let Ok(urdf_module) = urdf_module {
            ...
            urdf_module.links.iter().enumerate().for_each(|(i, x)| {
                ...
                progress_bar.update_preset(progress * 100.0);
                if x.visual.len() > 0 {
...
impl RobotPreprocessorModule for ApolloOriginalMeshesModule {
...
pub trait OriginalMeshesModuleGetFullPaths {
    fn get_full_paths(&self, s: &RobotPreprocessorSingleRobotDirectory) -> Vec<Option<PathBuf>>;
}
