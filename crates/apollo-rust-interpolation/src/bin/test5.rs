use apollo_rust_interpolation::splines::{get_interpolation_range, BSpline};
...
fn main() {
    ...
    println!(
        "Interpolating {} points, time consumed by De Boor's algorithm is {}s",
        ((control_points.len() - k + 1) as f64 / 0.00001).round(),
    ...
    println!(
        "Interpolating {} points, time consumed by fast Cox De Boor's recursion is {}s",
        ((control_points.len() - k + 1) as f64 / 0.00001).round(),
        time.elapsed().as_secs_f64()
    );
    ...
    println!(
        "Interpolating {} points, time consumed by naive Cox De Boor's recursion is {}s",
        ((control_points.len() - k + 1) as f64 / 0.00001).round(),
        time.elapsed().as_secs_f64()
    );
