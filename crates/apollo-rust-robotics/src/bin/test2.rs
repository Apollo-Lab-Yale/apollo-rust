...
fn main() {
    ...
    let mut s = c
        .link_shapes_module
        .get_shapes(LinkShapeMode::Full, LinkShapeRep::ConvexHull)
        .clone();
    s.extend(
        c.link_shapes_module
            .get_shapes(LinkShapeMode::Decomposition, LinkShapeRep::OBB)
            .clone(),
    );
    s.extend(
        c.link_shapes_module
            .get_shapes(LinkShapeMode::Decomposition, LinkShapeRep::BoundingSphere)
            .clone(),
    );
    s.extend(
        c.link_shapes_module
            .get_shapes(LinkShapeMode::Full, LinkShapeRep::OBB)
            .clone(),
    );
