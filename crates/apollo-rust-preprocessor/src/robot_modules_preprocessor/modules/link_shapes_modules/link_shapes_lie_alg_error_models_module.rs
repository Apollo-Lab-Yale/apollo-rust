/*
use apollo_rust_proximity::proxima::proxima2::{get_lladis_taylor_series_error_dataset_shape_pair_parallel, LieAlgMode, PolynomialFit, quantile_optimization};
use apollo_rust_robot_modules::{ResourcesSubDirectory, ResourcesType};
...
impl PreprocessorModule for ApolloLinkShapesLieAlgErrorModelsModule {
    ...
    fn build_raw(s: &ResourcesSubDirectory, progress_bar: &mut ProgressBarWrapper) -> Result<Self, String> {
        ...
        match s.resources_type {
            ResourcesType::Robot => {  }
            ResourcesType::Environment => {
                progress_bar.done_preset();
                return Ok(Self {
                    full_convex_hulls_standard_models: vec![],
                    full_obbs_standard_models: vec![],
                    full_bounding_spheres_standard_models: vec![],
                    decomposition_convex_hulls_standard_models: vec![],
                    decomposition_obbs_standard_models: vec![],
                    decomposition_bounding_spheres_standard_models: vec![],
                    full_convex_hulls_pseudo_models: vec![],
                    full_obbs_pseudo_models: vec![],
                    full_bounding_spheres_pseudo_models: vec![],
                    decomposition_convex_hulls_pseudo_models: vec![],
                    decomposition_obbs_pseudo_models: vec![],
                    decomposition_bounding_spheres_pseudo_models: vec![],
                })
            }
        }
        ...
        let mut count = 0.0;
        let num_loops = 3.0 * (num_full_shapes as f64 * num_full_shapes as f64) + 3.0 * (num_decomposition_shapes as f64 * num_decomposition_shapes as f64);
        ...
        for (idx, shape_vec) in shape_vecs.iter().enumerate() {
            ...
            for i in 0..num_shapes {
                'l: for j in 0..num_shapes {
                    count += 1.0;
                    progress_bar.update_with_percentage_preset((count / num_loops) * 100.0);
                    if j >= i { continue 'l; }

                    let shape_a = &shape_vec[i];
                    let shape_b = &shape_vec[j];

                    let ds1 = get_lladis_taylor_series_error_dataset_shape_pair_parallel(shape_a, shape_b, LieAlgMode::Standard, 10, 500, 2.0, 3.0);
                    let ds2 = get_lladis_taylor_series_error_dataset_shape_pair_parallel(shape_a, shape_b, LieAlgMode::Pseudo, 10, 500, 2.0, 3.0);
