...
fn main() {
    ...
    RobotProximityFunctions::self_distance(c.link_shapes_module(), &link_poses, link_shape_mode, link_shape_rep, None, false);
    let stop1 = start.elapsed();
    println!("{:?}", stop1);

    let start = Instant::now();
    // c.self_intersect_from_state(&state, LinkShapeMode::Full, LinkShapeRep::BoundingSphere, false);.
    RobotProximityFunctions::self_distance(c.link_shapes_module(), &link_poses, link_shape_mode, link_shape_rep, Some(c.link_shapes_skips_nalgebra_module.get_skips(link_shape_mode, link_shape_rep)), false);
