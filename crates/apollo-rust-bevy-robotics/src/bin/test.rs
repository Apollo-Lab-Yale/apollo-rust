...
use apollo_rust_bevy::apollo_bevy_utils::visibility::{BaseVisibility, VisibilityEngine, VisibilityRequestType, VisibilityRequest};
...
fn main() {
    ...
    app.add_systems(Update, |keys: Res<ButtonInput<KeyCode>>, mut engine: ResMut<VisibilityEngine>| {
        if keys.just_pressed(KeyCode::KeyM) {
            ...
            engine.add_base_change_request(VisibilityRequest::new(VisibilityRequestType::Toggle, Signature::ChainLinkMesh { components: ChainMeshComponents::new(vec![ChainMeshComponent::ChainInstanceIdx(1), ChainMeshComponent::LinkIdx(3), ChainMeshComponent::SubcomponentIdx(0), ChainMeshComponent::ChainMeshesRepresentation(ChainMeshesRepresentation::Plain)]) }))
        }
    });
