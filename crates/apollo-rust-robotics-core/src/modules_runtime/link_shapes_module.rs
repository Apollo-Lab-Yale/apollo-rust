...
use apollo_rust_proximity_parry::offset_shape::{
    to_offset_shape_bounding_sphere, to_offset_shape_obb, OffsetShape,
};
...
impl ApolloLinkShapesModule {
        ...
        convex_hull_meshes_module
            ...
            .for_each(|(link_idx, x)| {
                if let Some(path_buf) = x {
                    let full_path = s.resolve_path(path_buf);
                    let tm = load_stl_file(&full_path)
                        .expect(&format!("error: {:?}", full_path))
                        .to_trimesh();
                    let points: Vec<Point<f64>> = tm
                        .points()
                        .iter()
    ...
    */
    ...
    #[inline(always)]
    pub fn link_poses_to_shape_poses(
        &self,
        link_poses: &Vec<ISE3q>,
    ...
    pub fn get_shapes(
        ...
        match (link_shape_mode, link_shape_rep) {
            ...
            (LinkShapeMode::Decomposition, LinkShapeRep::ConvexHull) => {
                &self.decomposition_convex_hulls
            }
            (LinkShapeMode::Decomposition, LinkShapeRep::OBB) => &self.decomposition_obbs,
            (LinkShapeMode::Decomposition, LinkShapeRep::BoundingSphere) => {
                &self.decomposition_bounding_spheres
            }
        }
    }
    ...
    #[inline(always)]
    pub fn get_link_idx_and_subcomponent_idx_from_shape_idx(
        &self,
        shape_idx: usize,
        link_shape_mode: &LinkShapeMode,
    ) -> (usize, usize) {
        return match link_shape_mode {
            LinkShapeMode::Full => (self.full_shape_idx_to_link_idx[shape_idx].clone(), 0),
            LinkShapeMode::Decomposition => {
    ...
    pub fn get_shape_idx_from_link_idx_and_subcomponent_idx(
        &self,
        link_idx: usize,
        ...
        return match link_shape_mode {
            LinkShapeMode::Full => self.link_idx_to_full_shape_idx[link_idx].clone(),
            LinkShapeMode::Decomposition => {
