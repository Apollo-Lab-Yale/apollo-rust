use apollo_rust_bevy::apollo_bevy_utils::chain::{
    BevyChainProximityVisualizer, BevyChainSlidersEgui, ChainMeshesRepresentation,
};
...
use apollo_rust_bevy::apollo_bevy_utils::visibility::{
    VisibilityChangeEngine, VisibilityChangeRequest, VisibilityChangeRequestType,
};
...
use bevy::prelude::{
    App, AppExit, ButtonInput, EventWriter, IntoSystemConfigs, KeyCode, Res, ResMut, SystemSet,
    Update,
};
...
fn main() {
    ...
    let mut app = App::new().apollo_bevy_robotics_base(false, false);

    app = app.apollo_bevy_spawn_chain(
        &chain,
        0,
        ISE3q::new(I3::from_slices_euler_angles(&[0., 0., 0.], &[0., 0., 0.])),
    ...
    app.add_systems(
        ...
        |keys: Res<ButtonInput<KeyCode>>,
         mut engine: ResMut<VisibilityChangeEngine>,
         _color: ResMut<ColorChangeEngine>,
         mut exit: EventWriter<AppExit>| {
            if keys.just_pressed(KeyCode::KeyM) {
                ...
                engine.add_base_change_request(VisibilityChangeRequest::new(
                    VisibilityChangeRequestType::Toggle,
                    Signature::ChainLinkMesh {
                        components: ChainMeshComponents::new(vec![
                            ChainMeshComponent::ChainInstanceIdx(0),
                            ChainMeshComponent::LinkIdx(3),
