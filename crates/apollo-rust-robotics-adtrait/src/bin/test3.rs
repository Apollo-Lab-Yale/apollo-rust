...
impl<A: AD> DifferentiableFunctionTrait<A> for Test<A> {
    fn call(&self, inputs: &[A], _freeze: bool) -> Vec<A> {
        ...
        let target_pose = LieGroupISE3q::new(I3::from_slices_euler_angles(&[0.3.into(), 0.0.into(), 1.15.into()], &[0.0.into(), 0.0.into(), 0.0.into()]));
...
fn main() {
    ...
    let db1 = DifferentiableBlock::new_with_tag(TestDC, ReverseAD::new(), Test { robot: c1.clone() }, Test { robot: c2.clone() });
    let db2 = DifferentiableBlock::new_with_tag(TestDC, ForwardAD::new(), Test { robot: c1.clone() }, Test { robot: c3.clone() });
