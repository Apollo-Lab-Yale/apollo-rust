...
impl<P: ApolloPathBufTrait + Clone> PreprocessorModule<P> for ApolloFirstLookVisModule {
    ...
    fn build_raw(
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            if output.status.success() {
                ...
                return if res {
                    ...
                    Err(format!(
                        "First look vis module for chain {:?} was not verified.",
                        s.name
                    ))
                };
            }
            ...
            return Err(format!(
                "First look vis module for chain {:?} could not be built.",
                s.name
            ));
        }
        ...
        {
            return Err("WASM execution for FirstLookVisModule is not yet implemented (requires Web Worker or async function call)".to_string());
        }
    }
}
