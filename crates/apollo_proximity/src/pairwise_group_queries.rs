...
macro_rules! create_pairwise_group_query {
    ($func_name: ident, $output_type: ty, $query_func_code: expr, $early_stop_code: expr, $extra_args: ty) => {

        pub fn $func_name(group_a: &Vec<&dyn Shape>, poses_a: &Vec<ISE3q>, group_b: &Vec<&dyn Shape>, poses_b: &Vec<ISE3q>, skip_symmetrical_entries: bool, skips: Option<DMatrix<bool>>, early_stop: bool, extra_args: $extra_args) -> Vec<((usize, usize), $output_type)> {
            ...
            for (i, (sa, pa)) in group_a.iter().zip(poses_a).enumerate() {
                'l: for (j, (sb, pb)) in group_b.iter().zip(poses_b).enumerate() {
                    ...
                    let res = $query_func_code(pa, *sa, pb, *sb, &extra_args);
                    out.push( ((i,j), res) );
