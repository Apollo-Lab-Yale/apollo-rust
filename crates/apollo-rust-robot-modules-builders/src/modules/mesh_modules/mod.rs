...
use crate::RobotPreprocessorSingleRobotDirectory;

pub fn recover_full_paths_from_relative_paths(s: &RobotPreprocessorSingleRobotDirectory, paths: &Vec<Option<PathBuf>>) -> Vec<Option<PathBuf>> {
    let root = s.robots_directory.clone();
...
pub fn recover_full_paths_from_double_vec_of_relative_paths(s: &RobotPreprocessorSingleRobotDirectory, paths: &Vec<Vec<PathBuf>>) -> Vec<Vec<PathBuf>> {
    let root = s.robots_directory.clone();
...
macro_rules! create_generic_build_from_combined_robot {
    ($module:ty, $initial_push:expr) => {
        fn build_from_combined_robot(
            s: &RobotPreprocessorSingleRobotDirectory,
    ...
    ($module:ty) => {
        fn build_from_adjusted_robot(
            ...
            return if let Ok(adjusted_robot) = adjusted_robot {
                ...
                let ss = RobotPreprocessorRobotsDirectory::new(s.robots_directory.clone())
                    .get_robot_subdirectory(&adjusted_robot.base_robot_name);
                let module = <$module>::load_or_build(&ss, false).expect("error");
...
macro_rules! create_generic_build_raw {
    ($self_ty:ty, $default_fn:ident) => {
        fn build_raw(s: &RobotPreprocessorSingleRobotDirectory, progress_bar: &mut ProgressBarWrapper) -> Result<$self_ty, String> {
