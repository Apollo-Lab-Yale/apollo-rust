...
pub trait ConnectionsModuleBuilders: Sized {
    fn build_from_urdf_and_chain_modules(
        urdf_module: &ApolloURDFModule,
        chain_module: &ApolloChainModule,
        progress_bar_wrapper: &mut ProgressBarWrapper,
    ) -> Result<Self, String>;
}
    ...
    fn build_from_urdf_and_chain_modules(
        ...
        for i in 0..num_links {
            for j in 0..num_links {
                if i == j {
                    link_paths[i][j] = Some(vec![j])
                }

                let link_in_chain_i = &chain_module.links_in_chain[i];

                let mut stack: Vec<Vec<usize>> = link_in_chain_i
                    .children_link_idxs()
                    .iter()
                    .map(|x| vec![*x])
                    .collect();
                'l: while !stack.is_empty() {
                    ...
                    if link_in_chain.link_idx() == j {
                        link_paths[i][j] = Some(curr);
                        break 'l;
                    }
        ...
        Ok(Self {
            link_connection_paths: link_paths,
        })
    }
}
...
impl<P: ApolloPathBufTrait + Clone> PreprocessorModule<P> for ApolloConnectionsModule {
