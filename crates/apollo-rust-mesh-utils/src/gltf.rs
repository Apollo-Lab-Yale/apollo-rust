...
impl ToTrimeshScene for GltfInfo {
    fn to_trimesh_scene(&self) -> TrimeshScene {
        let mut out = TrimeshScene { nodes: vec![] };

        self.document.scenes().for_each(|scene| {
            let mut node_stack = vec![];
            scene
                .nodes()
                .for_each(|node| node_stack.push((node, None::<String>)));
            ...
            while !node_stack.is_empty() {
                ...
                mesh.primitives().for_each(|primitive| {
                    ...
                    let mut i = Vec::new();
                    if let Some(gltf::mesh::util::ReadIndices::U16(
                    ...
                    } else if let Some(gltf::mesh::util::ReadIndices::U32(
                        gltf::accessor::Iter::Standard(iter),
                    )) = reader.read_indices()
                    {
                        for v in iter {
                ...
                out.nodes.push(TrimeshNode {
                    ...
                    local_space_trimesh: TriMesh { points, indices },
                    offset_from_parent,
                    scale: [scale[0] as f64, scale[1] as f64, scale[2] as f64],
                });

                node.children().for_each(|child| match child.mesh() {
                    None => {}
...
impl ToMeshObjectScene for GltfInfo {
    fn to_mesh_object_scene(&self) -> MeshObjectScene {
        let mut out = MeshObjectScene { nodes: vec![] };
        ...
        self.document.scenes().for_each(|scene| {
            ...
            while !node_stack.is_empty() {
                ...
                let trimeshes: Vec<TriMesh> = points
                    .iter()
                    .zip(indices.iter())
                    .map(|(x, y)| TriMesh {
                        points: x.clone(),
...
pub fn load_gltf_file<P: ApolloPathBufTrait>(path: &P) -> Result<GltfInfo, String> {
    let (document, buffers, images) = gltf::import(path.to_path_buf()).expect("error");
    Ok(GltfInfo {
        document,
        buffers,
        images,
        ...
        gltf_json::Material {
            ...
            pbr_metallic_roughness: PbrMetallicRoughness {
                base_color_factor: PbrBaseColorFactor(
                    self.pbr_metallic_roughness().base_color_factor(),
                ),
