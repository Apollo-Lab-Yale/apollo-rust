...
pub trait FromRonString: ToRonString + DeserializeOwned {
    fn from_ron_string(ron_str: &str) -> Self
    where
        Self: Sized,
    {
        let load: Result<Self, _> = ron::from_str(ron_str);
        return if let Ok(load) = load {
            load
        } else {
    ...
    fn from_ron_string_option(ron_str: &str) -> Option<Self>
    where
        Self: Sized,
    {
        let load: Result<Self, _> = ron::from_str(ron_str);
...
pub trait FromJsonString: ToJsonString + DeserializeOwned {
    fn from_json_string(json_str: &str) -> Self
    where
        Self: Sized,
    {
        ...
        return if let Ok(load) = load {
            ...
            panic!(
                "Could not load json string {:?} into correct type.",
                json_str
            );
        };
    }

    fn from_json_string_option(json_str: &str) -> Option<Self>
    where
        Self: Sized,
    {
        let load: Result<Self, _> = serde_json::from_str(json_str);
...
pub trait FromTomlString: ToTomlString + DeserializeOwned {
    fn from_toml_string(toml_string: &str) -> Self
    where
        Self: Sized,
    {
        ...
        return if let Ok(load) = load {
            ...
            panic!(
                "Could not load toml string {:?} into correct type.",
                toml_string
            );
        };
    }

    fn from_toml_string_option(toml_string: &str) -> Option<Self>
    where
        Self: Sized,
    {
        let load: Result<Self, _> = toml::from_str(toml_string);
...
pub trait FromYamlString: ToYamlString + DeserializeOwned {
    ...
    fn from_yaml_string_option(yaml_string: &str) -> Option<Self>
    where
        Self: Sized,
    {
        let load: Result<Self, _> = serde_yaml::from_str(yaml_string);
...
impl<T> FromYamlString for T where T: ToYamlString + DeserializeOwned {}
