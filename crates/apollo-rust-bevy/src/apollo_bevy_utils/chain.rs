use crate::apollo_bevy_utils::colors::{
    BaseColor, ColorChangeEngine, ColorChangeRequest, ColorChangeRequestType,
};
use crate::apollo_bevy_utils::egui::{set_cursor_is_over_egui_default, CursorIsOverEgui};
...
use crate::apollo_bevy_utils::signatures::{
    ChainMeshComponent, ChainMeshComponents, Signature, Signatures,
};
...
use apollo_rust_modules::robot_modules::link_shapes_modules::link_shapes_approximations_module::{
    ApolloLinkShapesApproximationsModule, BoundingSphereDescriptor, OBBDescriptor,
};
...
use apollo_rust_proximity_parry::double_group_queries::{
    ConvertToAverageDistancesTrait, DoubleGroupProximityQueryMode, DoubleGroupProximityQueryOutput,
    SortDoubleGroupProximityQueryOutputTrait,
};
use apollo_rust_proximity_parry::{ProximityLossFunction, ToIntersectionResult, ToProximityValue};
...
use apollo_rust_robotics_core::modules_runtime::link_shapes_module::{
    ApolloLinkShapesModule, LinkShapeMode, LinkShapeRep,
};
...
use bevy::prelude::{
    default, Changed, Commands, Component, Cuboid, Entity, Gizmos, Mesh, Query, Res, ResMut,
    Sphere, Time, Transform, Window as Window1, With, Without,
};
use bevy::window::PrimaryWindow;
use bevy_egui::egui::{
    Color32, ComboBox, RichText, ScrollArea, SidePanel, Slider, TopBottomPanel, Ui,
};
...
impl BevySpawnChainLinkApproximationsRaw {
    fn action_spawn_bounding_sphere_approximations(
        &self,
        ...
        d.iter().enumerate().for_each(|(link_idx, x)| {
            ...
            x.iter().enumerate().for_each(|(_subcomponent_idx, y)| {
                ...
                e
                    ...
                    .insert(ChainLinkMesh {
                        chain_instance_idx: self.chain_instance_idx,
                        link_idx,
                    })
                    .insert(LinkApproximatingShape)
                    .insert(OffsetFrame(ISE3q::new(I3::from_slices_euler_angles(
                        &y.offset_xyz,
                        &y.offset_rpy,
                    ))))
    ...
    fn action_spawn_obb_approximations(
        &self,
            ...
            x.iter().enumerate().for_each(|(_subcomponent_idx, y)| {
                let mut e = commands.spawn(PbrBundle {
                    mesh: meshes.add(Mesh::from(Cuboid {
                        half_size: TransformUtils::util_convert_z_up_v3_to_z_up_vec3(
                            V3::from_column_slice(&y.half_extents),
                        ),
                    })),
                    material: materials.add(StandardMaterial {
    ...
    pub fn action_spawn_all_chain_shape_approximations(
        &self,
        commands: &mut Commands,
        meshes: &mut ResMut<Assets<Mesh>>,
        materials: &mut ResMut<Assets<StandardMaterial>>,
        ...
        let d = self
            ...
            .to_vec_of_vecs();
        let e = self
            .action_spawn_bounding_sphere_approximations(&d, &fk_res, commands, meshes, materials);
            ...
            x.iter().enumerate().for_each(|(s, y)| {
                let original_set = vec![
                    ChainMeshComponent::ChainMeshesRepresentation(
                        ChainMeshesRepresentation::BoundingSphereFull,
                    ),
                    ChainMeshComponent::ChainInstanceIdx(self.chain_instance_idx),
                    ChainMeshComponent::LinkIdx(link_idx),
                    ChainMeshComponent::SubcomponentIdx(s),
        ...
        e.iter().enumerate().for_each(|(link_idx, x)| {
            x.iter().enumerate().for_each(|(s, y)| {
                let original_set = vec![
                    ChainMeshComponent::ChainMeshesRepresentation(
                        ChainMeshesRepresentation::BoundingSphereDecomposition,
                    ),
                    ChainMeshComponent::ChainInstanceIdx(self.chain_instance_idx),
                    ChainMeshComponent::LinkIdx(link_idx),
                    ChainMeshComponent::SubcomponentIdx(s),
    ...
    pub fn get_system(
        self,
    ) -> impl FnMut(Commands, ResMut<Assets<Mesh>>, ResMut<Assets<StandardMaterial>>) + 'static
    {
        move |mut commands: Commands,
              mut meshes: ResMut<Assets<Mesh>>,
              mut materials: ResMut<Assets<StandardMaterial>>| {
            self.action_spawn_all_chain_shape_approximations(
                &mut commands,
                &mut meshes,
                &mut materials,
            );
...
impl BevySpawnChainMeshesRaw {
        ...
        full_paths.iter().enumerate().for_each(|(link_idx, x)| {
            ...
            x.iter().for_each(|path| {
                ...
                let entity = match self.mesh_type {
                    ...
                    MeshType::OBJ => spawn_obj(
                        path.clone(),
                        Color::srgba(0.6, 0.6, 0.62, 1.0),
                        Some(pose),
                        commands,
                        asset_server,
                        materials,
                ...
                commands
                    .entity(entity)
                    .insert(BaseColor(Color::srgba(0.6, 0.6, 0.62, 1.0)));
            });
            out.push(tmp);
        });
    ...
    pub fn action_spawn_chain_meshes(
        &self,
        commands: &mut Commands,
        asset_server: &Res<AssetServer>,
        materials: &mut ResMut<Assets<StandardMaterial>>,
        ...
        let full_paths = match &self.mesh_type {
            MeshType::GLB => match &self.chain_meshes_representation {
                ChainMeshesRepresentation::Plain => self
                    .plain_meshes_module
                    .get_glb_full_paths(&self.resources_sub_directory)
                    .to_vec_of_vec_path_bufs(),
                ChainMeshesRepresentation::ConvexHull => self
                    .convex_hull_meshes_module
                    .get_glb_full_paths(&self.resources_sub_directory)
                    .to_vec_of_vec_path_bufs(),
                ChainMeshesRepresentation::ConvexDecomposition => self
            ...
            MeshType::OBJ => match &self.chain_meshes_representation {
                ChainMeshesRepresentation::Plain => self
                    .plain_meshes_module
                    .get_obj_full_paths(&self.resources_sub_directory)
                    .to_vec_of_vec_path_bufs(),
                ChainMeshesRepresentation::ConvexHull => self
                    .convex_hull_meshes_module
                    .get_obj_full_paths(&self.resources_sub_directory)
        ...
        let res = self.clone().spawn_chain_meshes_generic(
            &full_paths,
            &fk_res,
            commands,
            asset_server,
        ...
        match &self.chain_meshes_representation {
            ...
            ChainMeshesRepresentation::ConvexDecomposition => {
                res.iter().flatten().for_each(|x| {
                    commands
                        .entity(x.clone())
                        .insert(ChainLinkConvexDecompositionMesh);
                });
            }
            _ => {
                panic!("not handled here.")
        ...
        res.iter().enumerate().for_each(|(link_idx, x)| {
            x.iter().enumerate().for_each(|(i, y)| {
                ...
                let p = ChainMeshComponents::get_power_set_default(
                    self.chain_meshes_representation.clone(),
                    self.mesh_type,
                    self.chain_instance_idx,
                    link_idx,
                    i,
                );
                for pp in p {
    ...
    pub fn get_system(
        self,
    ) -> impl FnMut(Commands, Res<AssetServer>, ResMut<Assets<StandardMaterial>>) + 'static {
        move |mut commands: Commands,
              asset_server: Res<AssetServer>,
              mut materials: ResMut<Assets<StandardMaterial>>| {
    ...
    */
    ...
    pub fn get_system(
        ...
        let raw = BevySpawnChainMeshesRaw {
            ...
            convex_decomposition_meshes_module: self
                .chain
                .convex_decomposition_meshes_module
                .clone(),
...
impl BevyChainSlidersEguiRaw {
                ...
                ui.group(|ui| {
                    ...
                    ScrollArea::vertical().max_height(400.0).show(ui, |ui| {
                        joints_in_chain.iter().for_each(|joint_in_chain| {
                            ...
                            let dof_idxs =
                                &dof_module.joint_idx_to_dof_idxs_mapping[joint_in_chain.joint_idx];
                            ...
                            ui.label(format!(
                                "Joint {}: {}",
                                joint_in_chain.joint_idx, joint.name
                            ));
                            ui.label(format!(
                                "Joint type {:?}, num dofs: {}",
                                joint.joint_type,
                            ...
                            for dof_idx in dof_idxs {
                                ...
                                if hovered && color_changes {
                                    color_change_engine.add_momentary_request(
                                        ColorChangeRequest::new(
                                            ColorChangeRequestType::medium_priority_color(
                                                0.0, 0.2, 1.0, 0.8,
                                            ),
                                            Signature::new_chain_link_mesh(vec![
                                                ...
                                                ChainMeshComponent::LinkIdx(parent_link_idx),
                                            ]),
                                        ),
                                    );
                                    color_change_engine.add_momentary_request(
                                        ColorChangeRequest::new(
                                            ...
                                            Signature::new_chain_link_mesh(vec![
                                                ChainMeshComponent::LinkIdx(child_link_idx),
                                                ChainMeshComponent::ChainInstanceIdx(
                                                    chain_instance_idx,
                                                ),
                                            ]),
                    ...
                    if ui.button("Copy state to clipboard").clicked() {
                        ...
                        #[cfg(not(target_arch = "wasm32"))]
                        {
                            ...
                            clipboard
                                .set_text(formated_string.to_string())
                                .expect("error");
                        }
                    }
                });
    ...
    pub fn action_chain_sliders_egui(
        &self,
        state_text_edit_string: &mut String,
        color_change_engine: &mut ResMut<ColorChangeEngine>,
        query: &mut Query<&mut ChainState>,
        ...
        Self::action_chain_sliders_egui_static(
            self.chain_instance_idx,
            &self.urdf_module,
            &self.chain_module,
            &self.dof_module,
    ...
    pub fn get_system_side_panel_left(
        self,
    ) -> impl FnMut(
        EguiContexts,
        Query<&mut ChainState>,
        ResMut<CursorIsOverEgui>,
        Query<&Window1, With<PrimaryWindow>>,
        ResMut<ColorChangeEngine>,
    ) + 'static {
        let mut state_text_edit_string = "".to_string();
        return move |mut egui_contexts: EguiContexts,
                     mut query: Query<&mut ChainState>,
                     mut cursor_is_over_egui: ResMut<CursorIsOverEgui>,
                     query2: Query<&Window1, With<PrimaryWindow>>,
                     mut color_change_engine: ResMut<ColorChangeEngine>| {
            let self_clone = self.clone();
            SidePanel::left(format!(
                "chain_sliders_side_panel_chain_instance_idx_{}",
                self.chain_instance_idx
            ))
            .show(egui_contexts.ctx_mut(), |ui| {
                self_clone.action_chain_sliders_egui(
                    &mut state_text_edit_string,
...
impl BevyChainSlidersEgui {
    pub fn action_chain_sliders_egui(
        ...
        BevyChainSlidersEguiRaw::action_chain_sliders_egui_static(
            self.chain_instance_idx,
            &self.chain.urdf_module,
            &self.chain.chain_module,
            &self.chain.dof_module,
            &self.chain.bounds_module,
            self.color_changes,
            state_text_edit_string,
            color_change_engine,
            query,
...
impl BevyChainLinkVisibilitySelectorRaw {
    ...
    pub fn action_chain_link_visibility_selector_static(
        chain_instance_idx: usize,
        urdf_module: &ApolloURDFNalgebraModule,
        chain_module: &ApolloChainModule,
        dof_module: &ApolloDOFModule,
        ...
        ui.group(|ui| {
            ScrollArea::vertical()
                ...
                .max_height(400.0)
                .show(ui, |ui| {
                    for (link, link_in_chain) in
                        urdf_module.links.iter().zip(&chain_module.links_in_chain)
                    {
                        ...
                        ui.checkbox(
                            &mut convex_hull_meshes_obj_visibility[link_idx],
                            "Convex shape",
                        );
                        ui.checkbox(
                            &mut convex_decomposition_meshes_obj_visibility[link_idx],
                            "Convex decomp.",
                        );
                        ui.checkbox(
                            &mut bounding_spheres_full_visibility[link_idx],
                            "Bounding sphere full",
                        );
                        ui.checkbox(&mut obbs_full_visibility[link_idx], "OBB full");
                        ui.checkbox(
                            &mut bounding_spheres_decomposition_visibility[link_idx],
                            "Bounding sphere decomp.",
                        );
                        ...
                        visibility_change_engine.add_base_change_request(
                            VisibilityChangeRequest::new(
                                VisibilityChangeRequestType::new_from_bool(b),
                                Signature::new_chain_link_mesh(vec![
                                    ChainMeshComponent::ChainInstanceIdx(chain_instance_idx),
                                    ChainMeshComponent::LinkIdx(link_idx),
                                    ChainMeshComponent::ChainMeshesRepresentation(
                                        ChainMeshesRepresentation::Plain,
                                    ),
                        ...
                        if b {
                            ...
                            let frame_vectors =
                                frame.0.rotation.to_rotation_matrix().frame_vectors();

                            let p1 = TransformUtils::util_convert_z_up_v3_to_y_up_vec3(
                                position + 0.2 * frame_vectors[0],
                            );
                            let p2 = TransformUtils::util_convert_z_up_v3_to_y_up_vec3(
                                position + 0.2 * frame_vectors[1],
                            );
                            let p3 = TransformUtils::util_convert_z_up_v3_to_y_up_vec3(
                                position + 0.2 * frame_vectors[2],
                            );

                            let center =
                                TransformUtils::util_convert_z_up_v3_to_y_up_vec3(position);
    ...
    pub fn action_chain_link_visibility_selector(
        &mut self,
        state: &V,
        gizmos: &mut Gizmos,
        ui: &mut Ui,
        ...
        Self::action_chain_link_visibility_selector_static(
            self.chain_instance_idx,
            &self.urdf_module,
            &self.chain_module,
            &self.dof_module,
            &mut self.plain_meshes_glb_visibility,
            &mut self.plain_meshes_obj_visibility,
            &mut self.convex_hull_meshes_obj_visibility,
            &mut self.convex_decomposition_meshes_obj_visibility,
            &mut self.bounding_spheres_full_visibility,
            &mut self.obbs_full_visibility,
            &mut self.bounding_spheres_decomposition_visibility,
            &mut self.obbs_decomposition_visibility,
            &mut self.frames_visibility,
            state,
            gizmos,
            ui,
            visibility_change_engine,
        );
    }
    ...
    pub fn get_system_side_panel_left(
        ...
        move |mut egui_contexts: EguiContexts,
              ...
              mut cursor_is_over_eugi: ResMut<CursorIsOverEgui>,
              query: Query<&Window1, With<PrimaryWindow>>,
              query2: Query<&ChainState>,
              mut gizmos: Gizmos| {
            let state = &query2
                .iter()
                .find(|x| x.chain_instance_idx == self.chain_instance_idx)
                .unwrap()
                .state;

            SidePanel::left(format!(
                "chain_link_visibility_selctor_side_panel_{}",
                self.chain_instance_idx
            ))
            .show(egui_contexts.ctx_mut(), |ui| {
                self_clone.action_chain_link_visibility_selector(
                    state,
                    &mut gizmos,
                    ui,
                    &mut visibility_change_engine,
                );
...
impl BevyChainLinkVisibilitySelector {
    pub fn get_system_side_panel_left(
        ...
        let c = BevyChainLinkVisibilitySelectorRaw::new(
            self.chain_instance_idx,
            self.chain.urdf_module.clone(),
            self.chain.chain_module.clone(),
            self.chain.dof_module.clone(),
        );
...
impl BevyChainStateUpdaterLoopRaw {
    pub fn action_pose_chain(
        &self,
        state: &V,
        global_offset: &ISE3q,
        query: &mut Query<(&mut Transform, &ChainLinkMesh), Without<LinkApproximatingShape>>,
    ) {
        let fk_res = RobotKinematicsFunctions::fk(
            state,
            &self.urdf_module,
        ...
        query.iter_mut().for_each(|(mut x, y)| {
            if y.chain_instance_idx == self.chain_instance_idx {
                ...
                *x = TransformUtils::util_convert_pose_to_y_up_bevy_transform(
                    &global_offset.group_operator(&fk_res[link_idx]),
                );
            }
        });
    }
    pub fn action_pose_chain_for_approximating_shapes(
        &self,
        state: &V,
        global_offset: &ISE3q,
        query: &mut Query<(&mut Transform, &ChainLinkMesh, &OffsetFrame)>,
    ) {
        let fk_res = RobotKinematicsFunctions::fk(
            state,
            &self.urdf_module,
        ...
        query.iter_mut().for_each(|(mut x, y, z)| {
            if y.chain_instance_idx == self.chain_instance_idx {
                let link_idx = y.link_idx;
                let t = global_offset
                    .group_operator(&fk_res[link_idx])
                    .group_operator(&z.0);
    ...
    pub fn get_system1(
        ...
        Query<(&mut Transform, &ChainLinkMesh), Without<LinkApproximatingShape>>,
        Query<&mut ChainState, Changed<ChainState>>,
    ) + 'static {
        return move |mut query1: Query<
            (&mut Transform, &ChainLinkMesh),
            Without<LinkApproximatingShape>,
        >,
                     query2: Query<&mut ChainState, Changed<ChainState>>| {
    ...
    pub fn get_system2(
        ...
        Query<(&mut Transform, &ChainLinkMesh, &OffsetFrame)>,
        Query<&mut ChainState, Changed<ChainState>>,
    ) + 'static {
        return move |mut query1: Query<(&mut Transform, &ChainLinkMesh, &OffsetFrame)>,
                     query2: Query<&mut ChainState, Changed<ChainState>>| {
            for x in query2.iter() {
                if x.chain_instance_idx == self.chain_instance_idx {
                    self.action_pose_chain_for_approximating_shapes(
                        &x.state,
                        &x.global_offset,
                        &mut query1,
                    );
...
impl BevyChainProximityVisualizerRaw {
    pub fn action_chain_proximity_visualizer_static(
        ui: &mut Ui,
        state_a: &V,
        state_b: &V,
        ...
        let fk_res_a =
            RobotKinematicsFunctions::fk(state_a, urdf_module_a, chain_module_a, dof_module_a);
        let fk_res_b =
            RobotKinematicsFunctions::fk(state_b, urdf_module_b, chain_module_b, dof_module_b);
        let res = RobotProximityFunctions::double_chain_contact(
            link_shapes_module_a,
            &fk_res_a,
            link_shape_mode_a.clone(),
            link_shape_rep_a.clone(),
            link_shapes_module_b,
            &fk_res_b,
            link_shape_mode_b.clone(),
            link_shape_rep_b.clone(),
            skips,
            false,
            f64::INFINITY,
            &double_group_proximity_query_mode,
        );
        let res = res.sort();
        ...
        ScrollArea::vertical()
            .id_source("proximity_visualizer1")
            .max_height(200.0)
            .show(ui, |ui| {
                Self::action_chain_proximity_visualizer_panel(
                    ui,
                    &fk_res_a,
                    &fk_res_b,
                    &res,
        ...
        match &average_res {
            Some(average_res) => {
                ...
                ScrollArea::vertical()
                    .id_source("proximity_visualizer2")
                    .max_height(200.0)
                    .show(ui, |ui| {
                        Self::action_chain_proximity_visualizer_panel(
                            ui,
        ...
        match &skips {
            ...
            Some(skips) => {
                ...
                ScrollArea::vertical()
                    .id_source("skips_visualizer")
                    .max_height(100.0)
                    .show(ui, |ui| {
                        Self::action_chain_skipped_pairs_visualizer(
                            ui,
                            chain_instance_idx_a,
                            chain_instance_idx_b,
                            link_shapes_module_a,
        ...
        let proximity_value =
            res.to_proximity_value(&ProximityLossFunction::Hinge { threshold: 0.7 }, 10.0);
        ui.separator();
        ui.horizontal(|ui| {
        ...
        match &average_res {
            ...
            Some(average_res) => {
                ui.separator();
                let proximity_value = average_res
                    .to_proximity_value(&ProximityLossFunction::Hinge { threshold: 0.7 }, 10.0);
                let color = if proximity_value < 0.33 {
                    Color32::GREEN
                } else if proximity_value <= 0.66 {
                    Color32::YELLOW
    ...
    pub fn action_chain_proximity_visualizer(
        &self,
        ui: &mut Ui,
        state_a: &V,
        state_b: &V,
        ...
        Self::action_chain_proximity_visualizer_static(
            ui,
            state_a,
            state_b,
            self.chain_instance_idx_a,
            ...
            &self.dof_module_a,
            &self.dof_module_b,
            &self.link_shapes_module_a,
            &self.link_shapes_module_b,
            self.double_group_proximity_query_mode.clone(),
            link_shape_mode_a,
            link_shape_mode_b,
            link_shape_rep_a,
            link_shape_rep_b,
    ...
    pub fn action_chain_proximity_visualizer_panel(
        ui: &mut Ui,
        _fk_res_a: &Vec<ISE3q>,
        _fk_res_b: &Vec<ISE3q>,
        ...
        res.outputs
            ...
            .for_each(|(_idx, (c, (i, j)))| {
                ...
                let link_idxs_a = link_shapes_module_a
                    .get_link_idx_and_subcomponent_idx_from_shape_idx(*i, link_shape_mode_a);
                let link_idxs_b = link_shapes_module_b
                    .get_link_idx_and_subcomponent_idx_from_shape_idx(*j, link_shape_mode_b);

                ui.label(format!(
                    "link {:?} <--> link {:?}",
                    link_idxs_a, link_idxs_b
                ));
                ...
                if ui
                    .radio(
                        selected_idxs.is_some()
                            && selected_idxs.unwrap() == (link_idxs_a, link_idxs_b),
                        "Select",
                    )
                    .clicked()
                {
                    if selected_idxs.is_none() {
                        *selected_idxs = Some((link_idxs_a, link_idxs_b));
                    } else if selected_idxs.unwrap() == ((link_idxs_a, link_idxs_b)) {
                        *selected_idxs = None;
                    } else {
                        *selected_idxs.as_mut().unwrap() = (link_idxs_a, link_idxs_b);
                    }
                };
                ui.horizontal(|ui| {
                    ui.label("distance: ");
                ...
                if selected_idxs.is_some() && selected_idxs.unwrap() == (link_idxs_a, link_idxs_b) {
                    color_change_engine.add_momentary_request(ColorChangeRequest::new(
                        ColorChangeRequestType::low_priority_alpha(0.2),
                        Signature::new_chain_link_mesh(vec![]),
                    ));
                    color_change_engine.add_momentary_request(ColorChangeRequest::new(
                        ColorChangeRequestType::high_priority_color(0.5, 0.7, 0.8, 0.85),
                        Signature::new_chain_link_mesh(vec![
                            ChainMeshComponent::ChainMeshesRepresentation(
                            ...
                            ChainMeshComponent::ChainInstanceIdx(chain_instance_idx_a),
                            ChainMeshComponent::LinkIdx(link_idxs_a.0),
                            ChainMeshComponent::SubcomponentIdx(link_idxs_a.1),
                        ]),
                    ));
                    color_change_engine.add_momentary_request(ColorChangeRequest::new(
                        ColorChangeRequestType::high_priority_color(0.8, 0.7, 0.4, 0.85),
                        Signature::new_chain_link_mesh(vec![
                            ChainMeshComponent::ChainMeshesRepresentation(
                                ChainMeshesRepresentation::from_link_shape_mode_and_link_shape_rep(
                                    link_shape_mode_b,
                            ...
                            ChainMeshComponent::ChainInstanceIdx(chain_instance_idx_b),
                            ChainMeshComponent::LinkIdx(link_idxs_b.0),
                            ChainMeshComponent::SubcomponentIdx(link_idxs_b.1),
                        ]),
                    ));
                    ...
                    gizmos.sphere(
                        ...
                        Quat::default(),
                        0.005,
                        Color::srgba(0.5, 0.7, 0.8, 0.95),
                    );
                    gizmos.sphere(
                        v2,
                        Quat::default(),
                        0.005,
                        Color::srgba(0.8, 0.7, 0.4, 0.95),
                    );
                }
            });
    }

    pub fn action_chain_skipped_pairs_visualizer(
        ui: &mut Ui,
        chain_instance_idx_a: usize,
        chain_instance_idx_b: usize,
        link_shapes_module_a: &ApolloLinkShapesModule,
    ...
    pub fn get_system_side_panel_left(
        ...
        move |mut egui_contexts: EguiContexts,
            ...
            let chain_state_a = query
                .iter()
                .find(|x| x.chain_instance_idx == self.chain_instance_idx_a)
                .expect("error");
            let chain_state_b = query
                .iter()
                .find(|x| x.chain_instance_idx == self.chain_instance_idx_b)
                .expect("error");
            ...
            visibility_change_engine.add_momentary_request(VisibilityChangeRequest::new(
                ...
                Signature::new_chain_link_mesh(vec![
                    ...
                    ChainMeshComponent::ChainInstanceIdx(self.chain_instance_idx_a),
                    ChainMeshComponent::MeshType(MeshType::OBJ),
                ]),
            ));
            visibility_change_engine.add_momentary_request(VisibilityChangeRequest::new(
                ...
                Signature::new_chain_link_mesh(vec![
                    ...
                    ChainMeshComponent::ChainInstanceIdx(self.chain_instance_idx_b),
                    ChainMeshComponent::MeshType(MeshType::OBJ),
                ]),
            ));
            visibility_change_engine.add_momentary_request(VisibilityChangeRequest::new(
            ...
            SidePanel::left("proximity_visualizer").show(egui_contexts.ctx_mut(), |ui| {
                ...
                self.action_chain_proximity_visualizer(
                    ui,
                    &chain_state_a.state,
                    &chain_state_b.state,
                    &link_shape_mode_a,
...
impl BevyChainProximityVisualizer {
    pub fn to_raw(&self) -> BevyChainProximityVisualizerRaw {
        BevyChainProximityVisualizerRaw {
            ...
            double_group_proximity_query_mode: if self.chain_instance_idx_a
                == self.chain_instance_idx_b
            {
...
impl BevyChainMotionPlaybackRaw {
    pub fn action_motion_playback_static(
        ...
        match playback_mode {
            ...
            PlaybackMode::Reversed => {
                *t -= delta_t * *speed;
                if *t <= 0.0 {
                    *t = interpolator.max_t() - t.abs() - 0.000001;
                }
            }
        }
        ...
        *state = ss.clone();
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .add(Slider::new(t, 0.0..=interpolator.max_t()).text("playback slider"))
                .changed()
            {
                *playback_mode = PlaybackMode::Paused;
            };
    ...
    pub fn action_motion_playback(
        ...
        Self::action_motion_playback_static(
            interpolator,
            t,
            speed,
            playback_mode,
    ...
    pub fn get_system_bottom_panel(
        self,
        interpolator: Arc<dyn InterpolatorTraitLite>,
    ) -> impl FnMut(
        EguiContexts,
...
impl ChainMeshesRepresentation {
    pub fn from_link_shape_mode_and_link_shape_rep(
        link_shape_mode: &LinkShapeMode,
        link_shape_rep: &LinkShapeRep,
    ) -> Self {
        match link_shape_mode {
            LinkShapeMode::Full => match link_shape_rep {
                LinkShapeRep::ConvexHull => Self::ConvexHull,
                LinkShapeRep::OBB => Self::OBBFull,
                LinkShapeRep::BoundingSphere => Self::BoundingSphereFull,
            },
            LinkShapeMode::Decomposition => match link_shape_rep {
                LinkShapeRep::ConvexHull => Self::ConvexDecomposition,
                LinkShapeRep::OBB => Self::OBBDecomposition,
                LinkShapeRep::BoundingSphere => Self::BoundingSphereDecomposition,
            },
        }
    }
}
